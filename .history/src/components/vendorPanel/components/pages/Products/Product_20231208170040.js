...
const Product = () => {
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const get_all_subcategory = async () => {
      try {
        const { data } = await axios.get(
          `${Baseurl}/api/v1/admin/subCategory/getAllsubCategory/${categoryId}`
        );
        setSubArr(data?.data);
      } catch (e) {
        console.log(e);
  ...
  function MyVerticallyCenteredModalEdit(props) {
    ...
    useEffect(() => {
      if (props.show === true) {
        ...
        setDescription(dataEdit.name)
        setPrice(dataEdit.name)
        setRating(dataEdit.name)
        setSize(dataEdit.name)
        setColor(dataEdit.name)
        setStock(dataEdit.name)
        setImage(dataEdit.name)
        setSubCategoryId(dataEdit.name)
      }
    }, [props]);
